...
const TwitterVideo = () => <iframe className="w-[22.25rem] h-[12.5rem] rounded-lg" src="https://www.youtube.com/embed/-rPbCeCbJVc?si=778KTA2eKneoQLDF" title="YouTube video player" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen></iframe>
...
const QuickButton: FC<{
  ...
  return <div className={`btn-gradient3`} onClick={() => {
    if (!activeAddress) {
      connectWallet()
    }
  }}>{activeAddress ? 'Join Competition' : 'Connect Wallet'}</div>
}

const ButtonShowMore: FC<{
  open: boolean, onClick: () => void
}> = ({ open, onClick }) => {
  return <div className=" absolute right-6 top-6
    btn-default flex items-center gap-2"
    onClick={onClick}>
    Show { open ? 'Less' : 'More'} <img src={ImgCompetition.ChevronDown} className={`w-3 h-3 ${open ? 'rotate-180' : ''}`} />
  </div>
}
...
const CompeittionDetails = [
  ...
  },
  {
    ...
    value: <Link to="/" className=" text-blue font-medium">https://github.com</Link>
  },
  {
...
const TableColumns = [
  ...
  },
  {
    ...
    key: 'operations',
    render: (_: string, item: any) => <div className="btn-default btn-small">Chat With AI Model</div>
  }
]
...
const JoinCompetitionModal: FC<{
  ...
  const [form] = useForm()
  return <Modal title={<div className="font-semibold text-black text-2xl">Submit Fine-tuned Model</div>} open={visible} onCancel={onCancel} onOk={onOk} footer={null}>
    <Form layout="vertical" form={form} className="my-10">
      <Form.Item name="modelName" label={<div className="text-xs text-black50">Model Name</div>}>
        <Input placeholder="Enter your model name (Example: AI model type A)" size="large" />
      </Form.Item>
      <Form.Item name="modelFile" label={<div className="text-xs text-black50 flex items-center gap-1">File Hash on Arweave 
        <img src={ImgCompetition.IconInfoCircle} className="w-3 h-3" /></div>}>
        <Input placeholder="Enter your File Hash on Arweave " size="large"  />
      </Form.Item>
    </Form>
    <div className="flex justify-end gap-4">
      <div className="btn-default w-32" onClick={() => {
        onCancel()
      }}>Cancel</div>
...
const Competition = () => {
  ...
  const [joinCompetitionModalVisible, setJoinCompetitionModalVisible] = useState(true)
  return <div id="competition" className='max-w-[1080px] mx-auto'>
    ...
    <div className="relative p-6 bg-light rounded-2xl">
      <div className="w-3/5">
        <h1 className="mb-6 font-bold text-3xl leading-tight">Fine-tune AI model_Conversations On Coding, Debugging, Storytelling</h1>
        <p className=" text-neutral-900 leading-none">Ends in 2 days 16 hours</p>
      </div>
      ...
      <div className={`mt-6 justify-between ${showMore ? 'flex' : 'hidden'}`}>
        <ul className="flex flex-col gap-4 w-3/5">
          {CompeittionDetails.map(({ icon, label, value }) => {
            return <li className="flex items-center gap-1 text-sm" key={label}>
          ...
          <li className="text-sm">
            <div className="flex items-center gap-1 mb-3">
              ...
              <span className="text-black">Descriptition:</span>
            </div>
            <p className="text-black50 px-5 leading-relaxed">Large language models (LLMs) are rapidly entering our lives, but ensuring their responses resonate with users is critical for successful interaction. This competition presents a unique opportunity to tackle this challenge with real-world data and help us bridge the gap between LLM capability and human preference.</p>
          </li>
        </ul>
        <TwitterVideo />
      </div>
    ...
    <div className="flex gap-4">
      {
        Statisitcs.map(({ label, value}) => {
          return <div className="flex-1 bg-light p-6 
            border border-black5 border-solid rounded-2xl" key={label}>
    ...
    <div className="flex flex-col my-6">
      ...
      <p className="mt-2 text-black50 text-xs">Leaderboard updated every 24 hours</p>
    </div>
    <div className="rounded-2xl pb-6">
      <Table dataSource={FakeData()} rowKey='model' columns={TableColumns} />
    </div>
    <JoinCompetitionModal visible={joinCompetitionModalVisible} onCancel={() => {
