...
import { Breadcrumb, Input, Spin, message } from "antd";
...
const Playground = () => {
  ...
  return (
    <div>
      <div
        className="mb-3 w-[680px] h-[385px] p-6 rounded-lg border-1 border-solid border-[#ebebeb] bg-white overflow-y-auto scroll-smooth"
        ref={scrollRef}
      >
      ...
      <div className="w-[680px] relative p-6 rounded-lg border-1 border-solid border-[#ebebeb] bg-white">
        <Input.TextArea
          value={prompt}
          onChange={(e) => {
        ...
        <div className={`absolute right-8 bottom-6`}>
          <Spin spinning={getTaskQuery.isFetching || addTaskMutation.isPending}>
            <img
              src={sendIcon}
              className={`w-[30px] h-[30px] ${latestRef ? "opacity-50" : ""}`}
              onClick={() => {
