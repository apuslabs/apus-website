...
export function addProcess(name: string, process_id: string): Promise<MessageResult> {
  return requestHB<MessageResult>(
    ANPM_DEFAULT_POOL,
    {
...
export function getBalance(user: string): Promise<string> {
  return getHBCache<Record<string, string>>(APUS_ADDRESS.Mint, "balances").then(
    (balanceMap) => balanceMap[user] || "0",
  );
}
...
export function getUserStake(user: string, pool_id: string): Promise<string> {
  return getHBCache<Record<string, Record<string, string>>>(ANPM_POOL_MGR, "stakers").then(
    (stakeMap) => stakeMap?.[user]?.[pool_id] || "0",
  );
}
...
export function getInterest(user: string): Promise<string> {
  return getHBCache<Record<string, string>>(ANPM_POOL_MGR, "distributed_interest").then(
    (interestMap) => interestMap[user] || "0",
  );
}
...
export function getProcesses(user: string): Promise<(Process & { process_id: string })[]> {
  return getHBCache<Record<string, Process>>(ANPM_DEFAULT_POOL, "process_ids").then((processes) => {
    return Object.entries(processes)
      .filter(([, value]) => value.created_by === user)
      .map(([key, value]) => Object.assign(value, { process_id: key }));
  });
}
