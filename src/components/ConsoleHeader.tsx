...
const UserInfo: FC = function() {
  ...
  return activeAddress ? <div className="btn-default">{ShortAddress(activeAddress)}</div> : <div className="btn-gradient3" onClick={connectWallet}>Connect Wallet</div>
}

export default function() {
  const location = useLocation()

  return <header className="bg-white h-20 w-full fixed top-0 left-0 right-0 px-10 flex items-center gap-10">
      ...
      <ul className="flex items-center gap-10">
        {
          ConsoleHeaderList.map(({ name, path }) => {
            return <li key={name} className={`text-black50 font-medium ${location.pathname === path ? 'text-gradient3' : ''}`}><Link to={path}>{name}</Link></li>
          })
        }
      </ul>
    </nav>
